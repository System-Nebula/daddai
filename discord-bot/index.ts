...
- If the user asks "what's my name?" and you find their name in the history, respond with something like "ur name is [NAME] ofc i remember!" 
...
- Keep your bubbly e-girl personality but make it personalized with information from the conversation history!`;
                    ...
                    try {
                        ...
                        const err = error as Error;
                        logger.warn(`⚠️ Failed to personalize response with memories: ${err.message}, using original RAG response`);
                        // Let the LLM handle name extraction from memories in the RAG response itself
                        // The memories are already included in the RAG context, so the LLM should be able to use them
                    }
                } else {
