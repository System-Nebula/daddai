...
class PersistentMemoryService extends EventEmitter {
    ...
    constructor() {
        ...
        this.httpPort = parseInt(process.env.MEMORY_SERVER_PORT || '8766', 10);
            ...
            for (const line of lines) {
                if (line.trim()) {
                    try {
                        ...
                        logger.error('[Memory] Failed to parse response:', { line, error: err.message });
                    }
                }
            }
        });
    ...
    handleResponse(response: MemoryResponse): void {
        ...
        const req = http.request(options, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    const err = error as Error;
                    logger.error('[Memory] Failed to parse HTTP response:', { error: err.message, data });
                    this.handleResponse({
                        id: request.id,
     ...
     */
    sendRequest(request: { method: string; params?: Record<string, unknown> }): Promise<unknown> {
        return new Promise((resolve, reject) => {
     ...
     */
    async getChannelMemories(channelId: string | null = null, limit = 100, channelName: string | null = null): Promise<unknown> {
     ...
     */
    async getAllChannels(): Promise<unknown> {
        try {
            const result = await this.sendRequest({
     ...
     */
    async getUserMemories(channelId: string, query: string, topK = 5, mentionedUserId: string | null = null): Promise<Array<{ id: string; content: string; metadata?: Record<string, unknown> }>> {
        try {
            const result = await this.sendRequest({
                method: 'search',
                params: {
            ...
            }) as { memories?: Array<{ id: string; content: string; metadata?: Record<string, unknown> }> };
     ...
     */
    async getAllMemories(limit = 1000): Promise<{ memories: Array<unknown>; count: number }> {
        try {
            const result = await this.sendRequest({
                method: 'get-all',
                params: {
            ...
            }) as { memories?: Array<unknown>; count?: number };
            return { memories: result.memories || [], count: result.count || 0 };
        } catch (error) {
