...
- Hybrid Neo4j + Elasticsearch support
"""
...
# Try to use hybrid store if Elasticsearch is enabled, otherwise use regular stores
try:
...
class EnhancedDocumentSearch:
    ...
    def _broad_retrieval(self,
        ...
        # Use hybrid search if available (Elasticsearch), otherwise vector search
        if hasattr(self.document_store, 'hybrid_search_shared'):
            try:
                shared_chunks = self.document_store.hybrid_search_shared(
                    query=query,
                ...
                logger.debug(f"Hybrid search failed, using vector search: {e}")
        ...
        if not doc_id and not doc_filename:
            # Use neo4j_store if available (for personal docs)
