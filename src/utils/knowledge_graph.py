...
class KnowledgeGraph:
    ...
    def get_user_document_history(self,
        ...
        with self.driver.session() as session:
            # Use separate OPTIONAL MATCH clauses to avoid warnings when relationship types don't exist
            # Only query relationships that are actually created (UPLOADED, QUERIED)
            result = session.run("""
                MATCH (u:User {id: $user_id})
                OPTIONAL MATCH (u)-[r1:UPLOADED]->(d1:SharedDocument)
                OPTIONAL MATCH (u)-[r2:QUERIED]->(d2:SharedDocument)
                WITH u, 
                     collect(DISTINCT {doc: d1, rel: r1, type: 'UPLOADED'}) + 
                     collect(DISTINCT {doc: d2, rel: r2, type: 'QUERIED'}) AS interactions
                UNWIND interactions AS interaction
                WITH interaction
                WHERE interaction.doc IS NOT NULL AND interaction.rel IS NOT NULL
                RETURN interaction.doc.id AS doc_id,
                       interaction.doc.file_name AS file_name,
                       interaction.type AS interaction_type,
                       toString(COALESCE(interaction.rel.last_interaction, datetime())) AS last_interaction,
                       COALESCE(interaction.rel.count, 0) AS interaction_count
                ORDER BY COALESCE(interaction.rel.last_interaction, datetime('1970-01-01')) DESC
                LIMIT $limit
            """,
                user_id=user_id,
                limit=limit
