...
class FileClassifier:
    ...
    SIGNATURES: List[Tuple[int, bytes, str, str, str]] = [
        ...
        (36865, b'CD001', 'disk_image', 'iso9660', 'ISO 9660 CD image'),
    ...
    def _analyze_text_content(cls, text: str, entropy: float, filepath: Path) -> Dict:
        ...
        # Check text file indicators
        if ext in cls.TEXT_INDICATORS:
            result = cls.TEXT_INDICATORS[ext]
            return {
                'category': result[0],
                'subtype': result[1],
                'notes': result[2]
    ...
    @classmethod
    def classify_file(cls, file_path: str) -> Dict[str, Any]:
        """
        ...
        Returns:
            ...
            {
                'category': str,      # Main category (document, image, archive, etc.)
                'subtype': str,       # Specific file type
                'notes': str,         # Description
                'is_safe': bool,      # Whether file is safe to process
                'is_executable': bool, # Whether file is an executable
                'is_encrypted': bool,  # Whether file appears encrypted
        ...
        if file_size > cls.MAX_FILE_SIZE:
            return {
                'category': 'unknown',
                'subtype': None,
                'notes': f'File too large ({file_size} bytes)',
                'is_safe': False,
                'is_executable': False,
                'is_encrypted': False,
                'entropy': 0.0,
        ...
        # Read header for signature analysis
        try:
        ...
        # Calculate entropy
        sample_size = min(len(header), cls.MAX_ENTROPY_SAMPLE_SIZE)
        entropy = cls.calculate_entropy(header[:sample_size])
        
        # Try signature-based detection first
        result = cls._detect_by_signature(header, filepath)
        
        if not result:
            # Try text-based detection
            try:
                text = header.decode('utf-8', errors='ignore')
                if len(text) > 10:  # Only if we got meaningful text
                    result = cls._analyze_text_content(text, entropy, filepath)
            except:
                pass
        
        if not result:
            # Unknown binary file
            if entropy > cls.ENCRYPTED_BINARY_THRESHOLD:
                result = {
                    'category': 'encrypted',
                    'subtype': None,
                    'notes': f'High entropy ({entropy:.2f}) - likely encrypted/compressed binary'
                }
            else:
                result = {
                    'category': 'unknown',
                    'subtype': None,
                    'notes': 'Unknown binary file'
                }
        
        # Determine safety flags
        is_executable = result.get('category') == 'executable'
        
        # Known safe formats that can have high entropy (compressed files like DOCX, PPTX are ZIP archives)
        known_safe_formats = [
            'document', 'text', 'text_scene', 'config', 'script',
            'archive',  # Archives are safe (DOCX, PPTX are ZIP archives)
            'image', 'audio', 'video',  # Media files
            'ebook', 'font', 'database', 'email', 'help', 'misc'
        ]
        
        # Only mark as encrypted if:
        # 1. Explicitly detected as encrypted category, OR
        # 2. High entropy AND not a known safe format
        is_encrypted = (result.get('category') in ['encrypted', 'encrypted_text'] or 
                       (entropy > cls.ENCRYPTED_BINARY_THRESHOLD and 
                        result.get('category') not in known_safe_formats))
        
        # Files are safe if they're in known safe categories and not executable/encrypted
        is_safe = (result.get('category') in known_safe_formats and 
                  not is_executable and 
                  not is_encrypted)
        
        return {
            'category': result.get('category', 'unknown'),
            'subtype': result.get('subtype'),
            'notes': result.get('notes', 'Unknown file type'),
            'is_safe': is_safe,
    ...
    @classmethod
    def is_file_safe_for_upload(cls, file_path: str) -> Tuple[bool, Optional[str]]:
        """
        Check if a file is safe to upload and process.
        ...
        try:
            ...
            # Known safe extensions that should be allowed even if classification is uncertain
            known_safe_extensions = [
                # Documents
                '.pdf', '.docx', '.doc', '.pptx', '.ppt', '.xlsx', '.xls',
                '.html', '.htm', '.adoc', '.asciidoc', '.rtf',
                # Text files
            ...
            classification = cls.classify_file(file_path)
            
            # If file has a known safe extension, be more lenient
            has_safe_extension = file_ext in known_safe_extensions
            
            if classification['is_executable']:
                return False, f"Executable files are not allowed: {classification.get('notes', 'Unknown executable')}"
            
            # For known safe extensions, don't reject based on encryption detection alone
            # (compressed files like DOCX have high entropy but aren't encrypted)
            if classification['is_encrypted'] and not has_safe_extension:
                return False, f"Encrypted files are not allowed: {classification.get('notes', 'Encrypted file detected')}"
            
            # If it's a known safe extension but classification failed, allow it anyway
            if not classification['is_safe']:
                if has_safe_extension:
                    # Log warning but allow the file
                    return True, None
                return False, f"Unsupported file type: {classification.get('notes', 'Unknown file type')}"
            
            return True, None
            
        except Exception as e:
            # If classification fails but file has known safe extension, allow it
            filepath = Path(file_path)
            file_ext = filepath.suffix.lower()
            known_safe_extensions = [
                '.pdf', '.docx', '.doc', '.pptx', '.ppt', '.xlsx', '.xls',
                '.html', '.htm', '.txt', '.md', '.json', '.csv', '.log',
                '.py', '.js', '.ts', '.java', '.cpp', '.c', '.sh', '.sql'
            ]
            if file_ext in known_safe_extensions:
                return True, None
