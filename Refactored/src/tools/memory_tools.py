...
Now async for better performance with database operations.
"""
...
class MemoryTools:
    ...
    async def save_core_memory(self, content: str, channel_id: str) -> str:
        """
        Save a core memory (important fact/preference) asynchronously.
        ...
        # Run embedding generation and storage in thread pool
        ...
        memory_id = await loop.run_in_executor(
            None,
            lambda: self.memory_store.store_memory(
    ...
    async def update_core_memory(self, memory_id: str, content: str, channel_id: str) -> str:
        """
        Update an existing core memory asynchronously.
    ...
    async def get_core_memory(self, channel_id: str, query: str = None, top_k: int = 5) -> list:
        """
        Retrieve core memories for a channel/user asynchronously.
        ...
        if query:
            query_embedding = await loop.run_in_executor(
                None,
                lambda: self.embedding_generator.generate_embedding(query)
            )
            memories = await loop.run_in_executor(
                None,
                lambda: self.memory_store.retrieve_relevant_memories(
                    channel_id=channel_id,
            ...
            try:
                memories = await loop.run_in_executor(
                    None,
                    lambda: self.memory_store.get_channel_memories(
                        channel_id=channel_id,
                ...
                query_embedding = await loop.run_in_executor(
                    None,
                    lambda: self.embedding_generator.generate_embedding("core memory")
                )
                memories = await loop.run_in_executor(
                    None,
                    lambda: self.memory_store.retrieve_relevant_memories(
