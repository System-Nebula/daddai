...
All tools are now async for better performance and concurrency.
"""
...
class LLMTool:
    ...
    def __init__(self,
        ...
        Args:
            ...
            function: Python function to execute (can be sync or async)
        """
        self.name = name
    ...
    async def execute(self, **kwargs) -> Any:
        """Execute the tool function (supports both sync and async functions)."""
        try:
            if self._is_async:
...
class LLMToolExecutor:
    ...
    async def execute_tool_calls(self, tool_calls: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        OPTIMIZED: Execute independent tools in parallel using asyncio.gather.
        """
        if len(tool_calls) == 1:
            return [await self.execute_tool_call(tool_calls[0])]
        ...
        # Execute independent tools in parallel using asyncio.gather
        if independent_tools:
            try:
                independent_results = await asyncio.gather(
                    *[self.execute_tool_call(tool_call) for tool_call in independent_tools],
                    return_exceptions=True
                )
                # Convert exceptions to error dicts
                for idx, result in enumerate(independent_results):
                    if isinstance(result, Exception):
                        logger.error(f"Error executing independent tool {idx}: {result}")
                ...
                for tool_call in independent_tools:
                    try:
                        ...
                        logger.error(f"Error executing independent tool: {e2}")
                        results.append({
                            "error": str(e2),
...
def create_rag_tools(pipeline) -> LLMToolRegistry:
    ...
    async def run_python(code: str) -> str:
        ...
        return await code_interpreter.run_python(code)
    ...
    async def analyze_image(image_url: str, question: str = None) -> Dict[str, Any]:
        """Analyze an image using vision capabilities."""
        return await vision_tool.process_image(image_url, question)
    ...
    async def save_core_memory(content: str, channel_id: str) -> str:
        """Save an important fact, user preference, or long-term memory."""
        return await memory_tools.save_core_memory(content, channel_id)
    ...
    async def get_core_memory(channel_id: str, query: str = None, top_k: int = 5) -> list:
    ...
    async def summarize_website_wrapper(url: str, max_length: int = 50000, save_to_documents: bool = True, force_refresh: bool = False) -> Dict[str, Any]:
        """
        ...
        result = await summarize_website(
            url=url,
    ...
    async def summarize_youtube_wrapper(url: str, language_codes: list = None, save_to_documents: bool = True, force_refresh: bool = False, user_id: str = None, channel_id: str = None) -> Dict[str, Any]:
        """
        ...
        # Let the tool function handle checking for existing documents and saving
        # Always pass components if available so tool can check cache and save new content
        result = await summarize_youtube(
            url=url,
