...
async def summarize_youtube(
    ...
    try:
        ...
        try:
            ...
            # Run YouTube API calls in thread pool since the library is sync
            ...
            def fetch_transcript_sync():
                ...
                return transcript_data, language_used, transcript_text
            
            # Execute sync function in thread pool
            result = await loop.run_in_executor(None, fetch_transcript_sync)
            
            # Handle error case
            if isinstance(result, tuple) and len(result) == 3 and isinstance(result[2], dict) and not result[2].get("success", True):
                return result[2]
            
            transcript_data, language_used, transcript_text = result
