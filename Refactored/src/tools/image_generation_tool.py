...
async def generate_image(
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=600.0) as client:
            response = await client.post(runsync_url, headers=headers, json=workflow)
            ...
            if result.get("status") in ["IN_QUEUE", "IN_PROGRESS"]:
                ...
                async with httpx.AsyncClient(timeout=60.0) as status_client:
                    while poll_count < max_polls:
                        await asyncio.sleep(5)
                        status_response = await status_client.get(status_url, headers=headers)
        ...
        if "output" in result:
            ...
            if img_type == "base64":
                ...
                # Decode base64 image to verify it's valid (run PIL operations in thread pool)
                try:
                    ...
                    image_path = await loop.run_in_executor(None, process_image_sync)
