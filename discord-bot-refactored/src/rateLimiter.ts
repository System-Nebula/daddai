...
interface RateLimitConfigs {
    ...
    channel: ChannelRateLimitConfig;
}
...
class RateLimiter {
    private rateLimits: Map<string, UserData>;
    private config: RateLimitConfigs;
     ...
     * @param type - Type of action: 'commands', 'messages', 'uploads'
     * @returns True if allowed, false if rate limited
     */
    checkUserLimit(userId: string, type: 'commands' | 'messages' | 'uploads'): boolean {
     ...
     */
    getRemaining(userId: string, type: 'commands' | 'messages' | 'uploads'): number {
        const limit = this.config.user[type];
     ...
     * @param userId - Discord user ID
     * @param type - Type of action
     * @returns Timestamp when limit resets
     */
    getResetTime(userId: string, type: 'commands' | 'messages' | 'uploads'): number {
        const userData = this.rateLimits.get(userId) || {};
