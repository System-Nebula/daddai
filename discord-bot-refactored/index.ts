...
import serverManager from './src/refactoredServerManager.js';
import refactoredRagService from './src/refactoredRagService.js';
...
// Use Refactored Agent Client by default
const useRefactoredAgent = true;
...
let gopherAgent: typeof refactoredAgentClient | null = null;

async function getGopherAgent() {
    if (!gopherAgent) {
        // Use RefactoredAgentClient for new architecture
        gopherAgent = refactoredAgentClient;
    }
    return gopherAgent;
}
...
// Initialize services
const ragService = refactoredRagService; // Use Refactored RAG Service (HTTP)
                ...
                (cmd.options as Array<{ type?: number; name?: string; description?: string; options?: unknown[]; required?: boolean }>).forEach((opt) => {
                    if (opt.type === 1) { // SUB_COMMAND
                        ...
                        if (opt.options && Array.isArray(opt.options) && opt.options.length > 0) {
                            (opt.options as Array<{ name?: string; required?: boolean }>).forEach((subOpt) => {
...
client.on(Events.MessageCreate, async (message: Message) => {
    ...
    try {
        ...
        if (hasAttachments) {
            ...
            const context: RouteMessageContext = {
                hasAttachments,
                isMentioned,
                recentMessages,
                userId,
...
function shouldUseAgenticModeLocal(message: string, intent: Record<string, unknown> | null): boolean {
    ...
    // Math/logic problems
    const mathIndicators = ["calculate", "solve", "compute", "what is", "how many", "how much", "analyze"];
    if (mathIndicators.some(indicator => messageLower.includes(indicator))) {
        // Check if it's a simple question (not complex)
        if (message.length < 20 && !messageLower.includes("complex") && !messageLower.includes("step")) {
            return false;
        }
        return true;
    }

    // Planning and creative tasks
    const planningIndicators = ["plan", "research", "design", "outline", "strategy", "generate", "create"];
...
async function handleQuestion(message: Message, routingResult: RoutingResult | null = null): Promise<void> {
    ...
    // Track the thinking message to update it or delete it later
    ...
    try {
        ...
        try {
            ...
            useAgenticMode = await Promise.race([
                agent.shouldUseAgenticMode(question, routingResult),
            ...
            if (err.message && err.message.includes('timeout')) {
                ...
                useAgenticMode = shouldUseAgenticModeLocal(question, intent as unknown as Record<string, unknown>);
            } else {
            ...
            try {
                ...
                if (agenticResult.status === 'success' && agenticResult.result) {
                    response = {
                        ...
                        source_documents: [],
                        source_memories: [],
                        tool_calls: agenticResult.tool_calls as any[]
                    };
